<template>
  ...
  <div class="input-container">
    <textarea
      ...
      type="text"
      placeholder="Escreva seu desejo aqui..."
      class="message-input"
      rows="4"
    />
...
const sendMessage = () => {
  ...
  if (messageInput.value.trim() !== '') {
    ...
    messageInput.value = ''
    // Rola para baixo após adicionar texto.
...
const handleKeyDown = event => {
  // Comentar as linhas abaixos para o enter quebrar linha em vez de enviar o texto.
  if (event.key === 'Enter' && !event.shiftKey) {
     event.preventDefault() // Evita inserção de nova linha.
   sendMessage()
  }
}
...
.messages {
  flex: 1;
  overflow-y: auto;
  padding-right: 20px; /* Ajusta a scrollbar para largura */
}
...
.bot-message {
  text-align: left;
  margin-left: 30px; /* Cria espaço para o Bot Icon */
}
